figma.showUI(__html__, { width: 480, height: 720, title: 'UX 플로우 생성기' });
...
figma.on('selectionchange', function () {
  if (figma.editorType !== 'figma') return; // FigJam에서는 벡터 화살표 미지원
...
figma.ui.onmessage = async (msg) => {
  ...
  if (msg.type === 'get-env') {
    figma.ui.postMessage({ type: 'env', editorType: figma.editorType });
  }

  if (msg.type === 'draw-ux-flow') {
    try {
      const section = await drawUxFlow(msg.flow);
      figma.currentPage.selection = [section];
      figma.viewport.scrollAndZoomIntoView([section]);
      figma.notify('✓ UX Flow 생성: ' + (msg.flow.title || section.name), { timeout: 3000 });
      figma.ui.postMessage({ type: 'ux-flow-drawn', title: msg.flow.title || section.name });
    } catch (e) {
      figma.ui.postMessage({ type: 'error', message: 'UX Flow 생성 실패: ' + e.message });
    }
  }
...
// ═══════════════════════════════════════════════════════════
// UX Flow (FigJam 전용) — 스킬이 만든 flow JSON을 도형+커넥터로 그린다
// ═══════════════════════════════════════════════════════════

var UXFLOW_LAYOUT = {
  COL_W: 340,      // 열 간격
  ROW_H: 320,      // 행 간격 (상세 텍스트 공간 포함)
  NODE_W: 240,
  NODE_H: 96,
  DECISION_W: 240,
  DECISION_H: 130,
  PAD: 80,         // 섹션 안쪽 여백
  HEADER_GAP: 40,  // 제목/링크와 노드 사이 간격
};

// 케이스별 색 — 저채도 파스텔 채움 + 1px 진한 테두리 (해피=회색, 에러=빨강, 예외=주황, API=파랑)
var UXFLOW_FILL = {
  happy:     { r: 0.965, g: 0.965, b: 0.973 }, // #F6F6F8
  error:     { r: 0.992, g: 0.925, b: 0.925 }, // #FDECEC
  exception: { r: 1.000, g: 0.957, b: 0.898 }, // #FFF4E5
  api:       { r: 0.918, g: 0.949, b: 0.996 }, // #EAF2FE
  decision:  { r: 1.000, g: 0.984, b: 0.922 }, // #FFFBEB
  start:     { r: 0.910, g: 0.969, b: 0.933 }, // #E8F7EE
};

var UXFLOW_STROKE = {
  happy:     { r: 0.690, g: 0.690, b: 0.710 }, // #B0B0B5
  error:     { r: 0.898, g: 0.282, b: 0.302 }, // #E5484D
  exception: { r: 0.910, g: 0.580, b: 0.040 }, // #E8940A
  api:       { r: 0.298, g: 0.553, b: 0.965 }, // #4C8DF6
  decision:  { r: 0.831, g: 0.655, b: 0.173 }, // #D4A72C
  start:     { r: 0.204, g: 0.659, b: 0.325 }, // #34A853
};

var UXFLOW_TEXT_COLOR = { r: 0.114, g: 0.114, b: 0.122 }; // #1D1D1F

var UXFLOW_EDGE = {
  yes:     { r: 0.204, g: 0.659, b: 0.325 }, // #34A853
  no:      { r: 0.898, g: 0.282, b: 0.302 }, // #E5484D
  default: { r: 0.557, g: 0.557, b: 0.576 }, // #8E8E93
};

function uxflowShapeType(nodeType) {
  if (nodeType === 'start' || nodeType === 'end') return 'ELLIPSE';
  if (nodeType === 'decision') return 'DIAMOND';
  if (nodeType === 'action') return 'ROUNDED_RECTANGLE';
  if (nodeType === 'api') return 'PARALLELOGRAM_RIGHT';
  return 'SQUARE'; // screen 등
}
...
function uxflowFill(node) {
  return UXFLOW_FILL[uxflowCaseKey(node)];
}

function uxflowStroke(node) {
  return UXFLOW_STROKE[uxflowCaseKey(node)];
}

// 도형/텍스트 공통 스타일: 1px 테두리 + Inter 폰트
function uxflowStyleShape(el, fillColor, strokeColor) {
  el.fills = [{ type: 'SOLID', color: fillColor }];
  el.strokes = [{ type: 'SOLID', color: strokeColor }];
  el.strokeWeight = 1;
  try {
    el.text.fontName = { family: 'Inter', style: 'Medium' };
    el.text.fills = [{ type: 'SOLID', color: UXFLOW_TEXT_COLOR }];
  } catch (e) { /* 일부 도형은 텍스트 스타일 미지원 */ }
}

// 노드 안 텍스트: 라벨 + 권한/상태/담당 뱃지
// 라벨 끝의 '*' → '(필수항목)', '(선택)' → '(선택항목)'으로 명확히 표기
function uxflowNodeLabel(node) {
  var label = String(node.label || '');
  if (/\*\s*$/.test(label)) return label.replace(/\s*\*\s*$/, '') + ' (필수항목)';
  if (/\(선택\)\s*$/.test(label)) return label.replace(/\s*\(선택\)\s*$/, '') + ' (선택항목)';
  return label;
}

function uxflowNodeText(node) {
  var lines = [uxflowNodeLabel(node)];
  var badges = [];
  if (node.owner) badges.push('[' + node.owner + ']');       // FE / BE / FE·BE
  if (node.role) badges.push('👤 ' + node.role);              // 권한
  if (node.state) badges.push('· ' + node.state);             // 상태
  if (badges.length) lines.push(badges.join(' '));
  return lines.join('\n');
}

// 범례: 도형 규칙을 플로우 상단에 항상 표시
function uxflowDrawLegend(section, x, y) {
  var items = [
    { shape: 'ELLIPSE',               label: '시작 · 종료',   key: 'start' },
    { shape: 'SQUARE',                label: '화면',          key: 'happy' },
    { shape: 'ROUNDED_RECTANGLE',     label: '액션 · 처리',   key: 'happy' },
    { shape: 'DIAMOND',               label: '분기 YES/NO',   key: 'decision' },
    { shape: 'PARALLELOGRAM_RIGHT',   label: 'API (BE)',      key: 'api' },
    { shape: 'ROUNDED_RECTANGLE',     label: '에러 경로',     key: 'error' },
    { shape: 'ROUNDED_RECTANGLE',     label: '예외 경로',     key: 'exception' },
  ];
  var caption = figma.createText();
  section.appendChild(caption);
  caption.fontName = { family: 'Inter', style: 'Medium' };
  caption.fontSize = 12;
  caption.characters = '범례';
  caption.fills = [{ type: 'SOLID', color: { r: 0.45, g: 0.45, b: 0.47 } }];
  caption.x = x;
  caption.y = y + 14;
  ...
  for (var i = 0; i < items.length; i++) {
    var it = items[i];
    var el = figma.createShapeWithText();
    section.appendChild(el);
    el.shapeType = it.shape;
    var w = it.shape === 'DIAMOND' ? 148 : 124;
    var h = it.shape === 'DIAMOND' ? 60 : 40;
    try { el.resize(w, h); }
    catch (e) { try { el.resizeWithoutConstraints(w, h); } catch (e2) { /* 기본 크기 */ } }
    el.x = cx;
    el.y = y + (it.shape === 'DIAMOND' ? -10 : 0);
    uxflowStyleShape(el, UXFLOW_FILL[it.key], UXFLOW_STROKE[it.key]);
    // FigJam 도형 내장 텍스트는 도형에 따라(마름모·평행사변형 등) 세로 중앙에서 밀린다.
    // 범례는 커넥터가 붙지 않으므로 텍스트를 비우고 별도 텍스트 노드를 정중앙에 오버레이한다.
    el.text.characters = '';
    var lgt = figma.createText();
    section.appendChild(lgt);
    lgt.fontName = { family: 'Inter', style: 'Regular' };
    lgt.fontSize = 11;
    lgt.characters = it.label;
    lgt.fills = [{ type: 'SOLID', color: UXFLOW_TEXT_COLOR }];
    lgt.x = el.x + (w - lgt.width) / 2;
    lgt.y = el.y + (h - lgt.height) / 2;
    lgt.name = '범례 텍스트: ' + it.label;
    el.name = '범례: ' + it.label;
    cx += w + 20;
    if (h > maxH) maxH = h;
  }
  ...
  // 입력 필수/선택 표기 규칙
  var conv = figma.createText();
  section.appendChild(conv);
  conv.fontName = { family: 'Inter', style: 'Medium' };
  conv.fontSize = 12;
  conv.characters = '(필수항목) 필수 입력 · (선택항목) 선택 입력';
  conv.fills = [{ type: 'SOLID', color: { r: 0.45, g: 0.45, b: 0.47 } }];
  try { conv.setRangeFills(0, 6, [{ type: 'SOLID', color: UXFLOW_STROKE.error }]); } catch (e) { /* 무시 */ }
  conv.x = cx + 8;
  conv.y = y + 12;
...
function uxflowEdgeColor(label) {
  if (!label) return UXFLOW_EDGE.default;
  var l = String(label).trim().toLowerCase();
  if (l === 'yes' || l === 'y' || l === '예' || l === '네' || l === 'ok' || l === '성공') return UXFLOW_EDGE.yes;
  if (l === 'no' || l === 'n' || l === '아니오' || l === '아니요' || l === '실패' || l === '오류') return UXFLOW_EDGE.no;
  return UXFLOW_EDGE.default;
}

async function drawUxFlow(flow) {
  if (figma.editorType !== 'figjam') {
    throw new Error('UX Flow는 FigJam(피그잼) 파일에서만 그릴 수 있습니다. 피그잼 파일을 열고 다시 실행하세요.');
  }
  if (!flow || !Array.isArray(flow.nodes) || flow.nodes.length === 0) {
    throw new Error('flow JSON에 nodes가 없습니다.');
  }
  ...
  title.fontSize = 22;
  title.fills = [{ type: 'SOLID', color: UXFLOW_TEXT_COLOR }];
  title.characters = flow.title || section.name;
  title.x = L.PAD * 0.5;
  title.y = cursorY;
  cursorY += title.height + 12;

  // 참고 링크: URL은 표기하지 않고 라벨 텍스트에만 하이퍼링크 — 위치는 섹션 크기 확정 후 우측 상단
  // docLinks(기준 스펙 md)를 먼저, figmaLinks(디자인)를 다음에 쌓는다
  var links = [];
  var docLinks = Array.isArray(flow.docLinks) ? flow.docLinks : [];
  for (var di = 0; di < docLinks.length; di++) {
    if (docLinks[di] && docLinks[di].url) links.push({ icon: '📄', label: docLinks[di].label || 'Spec', url: docLinks[di].url });
  }
  var figLinks = Array.isArray(flow.figmaLinks) ? flow.figmaLinks : [];
  for (var fi = 0; fi < figLinks.length; fi++) {
    if (figLinks[fi] && figLinks[fi].url) links.push({ icon: '🔗', label: figLinks[fi].label || 'Figma', url: figLinks[fi].url });
  }
  var linkTexts = [];
  for (var li = 0; li < links.length; li++) {
    ...
    lt.fontSize = 13;
    lt.characters = link.icon + ' ' + link.label;
    lt.fills = [{ type: 'SOLID', color: { r: 0.298, g: 0.553, b: 0.965 } }];
    try {
      lt.setRangeHyperlink(0, lt.characters.length, { type: 'URL', value: link.url });
      lt.setRangeTextDecoration(0, lt.characters.length, 'UNDERLINE');
    } catch (e) { /* 하이퍼링크 실패 시 텍스트만 유지 */ }
    linkTexts.push(lt);
  }

  // ── 범례 ──
  var legendH = uxflowDrawLegend(section, L.PAD * 0.5, cursorY + 16);
  cursorY += 16 + legendH;

  var nodesTop = cursorY + L.HEADER_GAP;

  // ── 노드 ──
  var shapeById = {};
  var gridById = {}; // id → {col, row} (커넥터 방향 결정용)
  var maxCol = 0, maxRow = 0;

  for (var i = 0; i < flow.nodes.length; i++) {
    var n = flow.nodes[i];
    var col = typeof n.col === 'number' ? n.col : i;
    var row = typeof n.row === 'number' ? n.row : 0;
    if (col > maxCol) maxCol = col;
    if (row > maxRow) maxRow = row;

    var cx = L.PAD + col * L.COL_W; // 각 셀의 왼쪽 기준
    var cy = nodesTop + row * L.ROW_H;

    var el;
    if (n.type === 'note') {
      el = figma.createSticky();
      section.appendChild(el);
      el.text.characters = (n.label ? n.label + '\n' : '') + (Array.isArray(n.details) ? n.details.join('\n') : '');
      try {
        el.text.fontName = { family: 'Inter', style: 'Regular' };
        el.text.fontSize = 12;
      } catch (se) { /* 스티키 텍스트 스타일 미지원 시 무시 */ }
      el.x = cx;
      el.y = cy;
    } else {
      var isDecision = n.type === 'decision';
      var w = isDecision ? L.DECISION_W : L.NODE_W;
      var h = isDecision ? L.DECISION_H : L.NODE_H;
      el = figma.createShapeWithText();
      section.appendChild(el);
      el.shapeType = uxflowShapeType(n.type);
      try { el.resize(w, h); }
      catch (re) { try { el.resizeWithoutConstraints(w, h); } catch (re2) { /* 기본 크기 유지 */ } }
      el.x = cx;
      el.y = cy + (isDecision ? -(h - L.NODE_H) / 2 : 0);
      uxflowStyleShape(el, uxflowFill(n), uxflowStroke(n));
      el.text.characters = uxflowNodeText(n);
      el.text.fontSize = 13;
      // '(필수항목)' 표기는 빨간색으로 강조
      var reqIdx = el.text.characters.indexOf('(필수항목)');
      if (reqIdx >= 0) {
        try { el.text.setRangeFills(reqIdx, reqIdx + 6, [{ type: 'SOLID', color: UXFLOW_STROKE.error }]); } catch (se) { /* 무시 */ }
      }
      el.name = n.label || n.id;

      // 상세 내용: 노드 아래 짧은 문장 나열
      if (Array.isArray(n.details) && n.details.length) {
        var dt = figma.createText();
        section.appendChild(dt);
        dt.fontName = { family: 'Inter', style: 'Regular' };
        dt.fontSize = 11;
        dt.characters = n.details.map(function (d) { return '· ' + d; }).join('\n');
        dt.fills = [{ type: 'SOLID', color: { r: 0.43, g: 0.43, b: 0.45 } }];
        dt.x = cx;
        dt.y = cy + L.NODE_H + 12;
        if (dt.width > L.NODE_W + 40) {
          dt.textAutoResize = 'HEIGHT';
          dt.resize(L.NODE_W + 40, dt.height);
        }
      }
    }
    shapeById[n.id] = el;
    gridById[n.id] = { col: col, row: row };
  }
  ...
  for (var ei = 0; ei < edges.length; ei++) {
    ...
    var sm = 'AUTO', em = 'AUTO';
    var fg = gridById[edge.from], tg = gridById[edge.to];
    if (fg && tg) {
    ...
    conn.connectorStart = { endpointNodeId: from.id, magnet: sm };
    conn.connectorEnd = { endpointNodeId: to.id, magnet: em };
    conn.connectorEndStrokeCap = 'ARROW_LINES';
    conn.strokes = [{ type: 'SOLID', color: uxflowEdgeColor(edge.label) }];
    conn.strokeWeight = 1;
    if (edge.label) {
      conn.text.characters = String(edge.label);
      try { conn.text.fontName = { family: 'Inter', style: 'Medium' }; } catch (ce) { /* 무시 */ }
  ...
  var linkY = L.PAD * 0.5;
  for (var lj = 0; lj < linkTexts.length; lj++) {
    var ltxt = linkTexts[lj];
    ltxt.x = secW - L.PAD * 0.5 - ltxt.width;
    ltxt.y = linkY;
    linkY += ltxt.height + 6;
  }
