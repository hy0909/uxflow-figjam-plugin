...
if [ -z "$CLAUDE_PATH" ] && [ -d "$HOME/Library/Application Support/Claude/claude-code" ]; then
  CLAUDE_PATH=$(find "$HOME/Library/Application Support/Claude/claude-code" -path "*/claude.app/Contents/MacOS/claude" -type f 2>/dev/null | sort -Vr | head -n 1)
fi
if [ -z "$CLAUDE_PATH" ] || [ ! -x "$CLAUDE_PATH" ]; then
  echo "❌ Claude Code CLI를 찾을 수 없습니다."
  echo "   Claude 앱/Claude Code가 설치되어 있는지 확인해주세요."
  exit 1
fi
