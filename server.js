...
// 실행: node server.js  (API 키 불필요, Claude Code 로그인 세션 사용)
...
function findClaudeCLI() {
  const appSupportDir = path.join(process.env.HOME || '', 'Library/Application Support/Claude/claude-code');
  try {
    const versions = fs.readdirSync(appSupportDir)
      .filter(name => /^\d+(?:\.\d+)+$/.test(name))
      .sort((a, b) => b.localeCompare(a, undefined, { numeric: true }));
    for (const version of versions) {
      const candidate = path.join(appSupportDir, version, 'claude.app/Contents/MacOS/claude');
      if (isExecutable(candidate)) return candidate;
    }
  } catch {
    // Claude Desktop/Code 설치 경로가 없으면 PATH 후보로 이어서 탐색.
  }
  ...
  try {
    const fromPath = execSync('which claude', { encoding: 'utf8' }).trim();
...
function getClaudeCLI(forceRefresh) {
  if (!forceRefresh && isExecutable(cachedClaudePath)) return cachedClaudePath;
  cachedClaudePath = findClaudeCLI();
  return cachedClaudePath;
}

const CLAUDE_PATH = getClaudeCLI(true);
if (!CLAUDE_PATH) {
  console.error('[오류] Claude Code CLI를 찾을 수 없습니다.');
  console.error('  Claude 앱/Claude Code가 설치되어 있는지 확인하거나 claude CLI를 PATH에 추가해주세요.');
  process.exit(1);
}
console.log(`[정보] Claude Code CLI: ${CLAUDE_PATH}`);
console.log(`[정보] Claude 모델: ${MODEL}`);
...
function callClaudeCLI(prompt, retriedAfterMissingCli) {
  return new Promise((resolve, reject) => {
    const claudePath = getClaudeCLI(!!retriedAfterMissingCli);
    if (!claudePath) {
      reject(new Error('Claude Code CLI를 찾을 수 없습니다. Claude 앱/Claude Code가 설치되어 있는지 확인하거나 claude CLI를 PATH에 추가해주세요.'));
      return;
    }
    ...
    proc.stdin.on('error', () => {
      // 프로세스가 빠르게 종료된 경우 close/error 핸들러에서 실제 오류를 처리.
    });
    ...
    proc.on('error', (err) => {
      clearTimeout(timer);
      if (err && err.code === 'ENOENT' && !retriedAfterMissingCli) {
        cachedClaudePath = null;
        callClaudeCLI(prompt, true).then(resolve, reject);
        return;
      }
      reject(new Error(`Claude Code CLI 실행 실패: ${err.message}`));
    });
  });
}
...
- 필수 입력 전부가 '필수 모두 입력?' decision으로 수렴 — 선택 입력은 수렴시키지 말고 details에 '입력 여부 검사 없음' 명시. 필수 라벨 끝 '*', 선택은 '(선택)'.
- 진입·버튼은 [ ] 표기 (예: GNB-[QR 발주서 작성]). 취소/닫기 종결은 end 타입.
- title은 '[대상/페이지] 기능 플로우' 형식 (예: '[QR 그룹] 수정 플로우') — 페이지 단위는 항상 [ ]로 감싼다.
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && req.url === '/health') {
    ...
    res.end(JSON.stringify({ ok: true, cli: getClaudeCLI(), model: MODEL }));
    return;
  }
  ...
  if (req.method === 'POST' && req.url === '/ux-flow') {
    ...
    req.on('end', () => {
      try {
        ...
        if (!flow.page || !flow.feature) throw new Error('page와 feature는 필수입니다.');
        if (!Array.isArray(flow.nodes) || flow.nodes.length === 0) throw new Error('nodes가 비어 있습니다.');
        if (!fs.existsSync(UXFLOW_DIR)) fs.mkdirSync(UXFLOW_DIR, { recursive: true });
        const slug = (flow.page + '-' + flow.feature)
          .replace(/[\/\\:*?"<>|\s]+/g, '-')
          .replace(/^-+|-+$/g, '')
          .slice(0, 80) || 'flow';
        const file = slug + '.json';
  ...
  if (req.method === 'GET' && req.url === '/ux-flow/list') {
    try {
      const flows = [];
      if (fs.existsSync(UXFLOW_DIR)) {
        for (const f of fs.readdirSync(UXFLOW_DIR)) {
          if (!f.endsWith('.json')) continue;
          const full = path.join(UXFLOW_DIR, f);
          try {
          ...
          } catch (pe) { /* 손상된 JSON은 목록에서 제외 */ }
        }
      }
      flows.sort((a, b) => b.mtime - a.mtime);
      res.writeHead(200, { 'Content-Type': 'application/json' });
  ...
  if (req.method === 'GET' && req.url.startsWith('/ux-flow/get')) {
    try {
      const u = new URL(req.url, 'http://localhost');
      const file = path.basename(u.searchParams.get('file') || ''); // 경로 탈출 방지
      const full = path.join(UXFLOW_DIR, file);
      if (!file.endsWith('.json') || !fs.existsSync(full)) throw new Error('플로우 파일을 찾을 수 없습니다: ' + file);
      res.writeHead(200, { 'Content-Type': 'application/json' });
