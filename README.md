...
2. md를 넘겨주기 — 둘 중 아무거나
   - GitHub md 링크 붙여넣기 (폴더(tree) 링크면 안에 있는 md를 전부 읽어요)
   - **md 파일을 드롭 영역에 끌어다 놓기** (여러 개 가능, 클릭해서 고를 수도 있어요)
3. 피그마 링크도 있으면 같이 붙여넣고 [⚡ 플로우 생성 → 그리기] 클릭

끝. 서버가 md를 읽고 Claude가 플로우를 만들어서 캔버스에 바로 그려줍니다.
공개 레포 링크는 gh CLI 없이도 읽고, 프라이빗 레포는 gh 로그인이 필요해요.
링크가 안 읽히면 md 파일을 그냥 끌어다 놓으면 됩니다.
